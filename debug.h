...
#ifndef PRINT_RESULT_FILE
#define PRINT_RESULT_FILE 0
#endif
...
#define ERROR(format, ...) do{PRINTFUNC(NVP_PRINT_FD, "\033[01;33mNVP_ERROR\e[m (pid %i): " format, getpid(), ##__VA_ARGS__); _nv_error_count++; if(SPIN_ON_ERROR){ _nvp_debug_handoff(); } }while(0)

FILE *debug_fd;
FILE *result_fd;
#define DEBUG_FD debug_fd
#define RESULT_FD result_fd

#if PRINT_RESULT_FILE
#define RESULT_FILE(format, ...) do {PRINTFUNC(RESULT_FD, "\033[01;33mNVP_RESULT\e[m (pid %i): " format, getpid(), ##__VA_ARGS__); }while(0)
#else
#define RESULT_FILE(format, ...) do{}while(0)
#endif

#if PRINT_DEBUG_FILE
#define DEBUG_FILE(format, ...) do {PRINTFUNC(DEBUG_FD, "\033[01;33mNVP_DEBUG\e[m (pid %i): " format, getpid(), ##__VA_ARGS__); }while(0)
#else
