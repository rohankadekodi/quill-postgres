...
uint64_t traverseTree(struct merkleBtreeNode *root, unsigned long mmap_start_addr) {
	...
	uint8_t level0_dirty_children[2048];
#endif
#ifdef LEVEL0_NUM_CHILDREN	
	uint8_t level0_total_dirty_children = 0;
	...
	while(__builtin_popcountll(data)) {
		...
		if((level0_total_dirty_children == 0) || (level0_dirty_children[level0_total_dirty_children-1] != effective_index/8)) {
			level0_dirty_children[level0_total_dirty_children] = (effective_index)/8;
	...
	for(i=0; i<level0_total_dirty_children; i++) {
		...
		while(__builtin_popcountll(data)) {
			...
			if((level1_total_dirty_children == 0) || (level1_dirty_children[level1_total_dirty_children-1] != effective_index/16)) {
				level1_dirty_children[level1_total_dirty_children] = effective_index/16;
	...
	for(i=0; i<level2_total_dirty_children; i++) {
		for(j=0; j<64; j++) {
			data = leaf->bitmaps[level2_dirty_children[i]*64+j];
			if(data) {
				leaf_dirty_index[leaf_total_dirty_indexes] = (level2_dirty_children[i]*64)+j;
