...
all: sofiles $(COMMON_OBJ) $(NVP_STACK) $(NVP_LRU_CACHE)

include common.mk

#sofiles: $(COMMON_OBJ) $(NVP_SOFILES) $(MONETA_SOFILES) $(SDSSD_SOFILES) $(BANKSHOT_SOFILES)
sofiles: $(COMMON_OBJ) $(NVP_SOFILES) $(NVP_STACK) $(NVP_LRU_CACHE) 
...
libnvp.so: fileops_hub.o $(COMMON_OBJ)
	$(CC) $(CLFAGS) -shared -Wl,-soname,$@ -o $@ $^ -ldl -lrt
...
libfileops_nvp.so: fileops_nvp.o $(COMMON_OBJ) $(NVP_STACK) $(NVP_LRU_CACHE)
	$(CC) $(CFLAGS) -shared -Wl,-soname,$@ -o $@ $< $(COMMON_OBJ) $(NVP_STACK) $(NVP_LRU_CACHE) -ldl -funroll-loops -L. -llfds711 -lrt
