...
RETT_EXECL ALIAS_EXECL(INTF_EXECL) WEAK_ALIAS("_hub_EXECL");
RETT_EXECL _hub_EXECL(INTF_EXECL);
				   
RETT_SHM_COPY _hub_SHM_COPY();
				      
#ifdef TRACE_FP_CALLS				 

//RETT_FDOPEN ALIAS_FDOPEN(INTF_FDOPEN) WEAK_ALIAS("_hub_FDOPEN");
//RETT_FDOPEN  _hub_FDOPEN(INTF_FDOPEN);
...
void _hub_init2(void)
{
	...
	{
		if(!strcmp(_hub_fileops_lookup[i]->name, "posix"))
		{
			MSG("Can't add fileop %s: one with the same name already exists at index %i. Ignoring. \n", "posix", i);
			goto out;
		}
	}
	...
	result_fd = _hub_find_fileop("posix")->FOPEN("/tmp/ledger_res.tmp", "a");
	
	DEBUG_FILE("%s: Created and opened debug file\n", __func__);
...
#define HUB_WRAP_HAS_FD(op)						\
	RETT_##op _hub_##op ( INTF_##op ) {				\
		...
		DEBUG_FILE("CALL: _hub_" #op ", fd = %i\n", file);		\
		if(file>=OPEN_MAX) { DEBUG("file descriptor too large (%i > %i)\n", file, OPEN_MAX-1); errno = EBADF; return (RETT_##op) -1; } \
		if(file<0) { DEBUG_FILE("file < 0 (file = %i).  return -1;\n", file); errno = EBADF; return (RETT_##op) -1; } \
		if(_hub_fd_lookup[file]==NULL) { DEBUG_FILE("_hub_"#op": That file descriptor (%i) is invalid: perhaps you didn't call open first. Assigning hub fileops\n", file); errno = EBADF; _hub_fd_lookup[file] = _hub_fileops; } \
...
#define HUB_WRAP_HAS_FP(op)						\
	RETT_##op _hub_##op ( INTF_##op ) {				\
		...
		if(_hub_fd_lookup[fileno(fp)]==NULL) { DEBUG_FILE("_hub_"#op": That file descriptor (%i) is invalid: perhaps you didn't call open first. Assigning hub fileops \n", fileno(fp)); errno = EBADF; _hub_fd_lookup[fileno(fp)] = _hub_fileops; } \
...
RETT_OPEN _hub_OPEN(INTF_OPEN)
{
	...
	{		
		if(FLAGS_INCLUDE(oflag, O_CREAT)) {
			DEBUG_FILE("File does not exist and is set to be created.  Using managed fileops (%s)\n",
			      _hub_managed_fileops->name);
			op_to_use = _hub_managed_fileops;
		} else {
			DEBUG_FILE("File does not exist and is not set to be created.  Using unmanaged fileops (%s)\n",
			      _hub_fileops->name);
			op_to_use = _hub_fileops;
		}
 ...
 opening:	
	...
	DEBUG_FILE("%s: path = %s. Flags = %d. (Read flag = %d, Write flag = %d, R/W flag = %d). Fd = %d\n", __func__, path, oflag, O_RDONLY, O_WRONLY, O_RDWR, result);
...
RETT_OPENAT _hub_OPENAT(INTF_OPENAT)
{
	...
	if (FLAGS_INCLUDE(oflag, O_CREAT)) {
			...
			int mode = va_arg(arg, int);
			va_end(arg);
			result = op_to_use->OPENAT(dirfd, path, oflag, mode);
	} else {
		result = op_to_use->OPENAT(dirfd, path, oflag);
	}
...
RETT_EXECVE _hub_EXECVE(INTF_EXECVE) {
	...
	int res = _hub_fileops->TRUNC64(exec_hub_fd, (1024*1024));
	if (res == -1) {
...
RETT_EXECL _hub_EXECL(INTF_EXECL) {
	...
	op_to_use = _hub_managed_fileops;
	result = op_to_use->EXECL(CALL_EXECL, argv1, argv2, (char *) NULL);
...
RETT_FDOPEN _hub_FDOPEN(INTF_FDOPEN)
{
	HUB_CHECK_RESOLVE_FILEOPS(_hub_, FDOPEN);
	
	FILE *fp = NULL;	
	int fd = -1, oflag = 0;
	int num_mode_chars = 0;
	struct Fileops_p *op_to_use = _hub_fileops;

	fp = op_to_use->FDOPEN(CALL_FDOPEN);
	if (fp != NULL)
		_hub_fd_lookup[fileno(fp)] = op_to_use;
...
RETT_FOPEN _hub_FOPEN(INTF_FOPEN)
{
	...
	*/
	...
	if (fd >= 0) {
		fp = fdopen(fd, mode);
		if (!fp) {
			DEBUG_FILE("%s: fdopen failed! error = %s, fd = %d, mode = %s\n", __func__, strerror(errno), fd, mode);
			assert(0);
		}
	}
...
RETT_FCLOSE _hub_FCLOSE(INTF_FCLOSE)
{
	...
	assert(_hub_fd_lookup[fileno(fp)]!=NULL);

	DEBUG("_hub_CLOSE is calling %s->CLOSE\n", _hub_fd_lookup[fileno(fp)]->name);
	
	RETT_CLOSE result = _hub_CLOSE(fileno(fp));
...
RETT_CLOSE _hub_CLOSE(INTF_CLOSE)
{
	...
	if( (file<0) || (file >= OPEN_MAX) ) {
		...
		DEBUG_FILE("fd %i is larger than the maximum number of open files; ignoring it.\n", file);
		errno = EBADF;
		return -1;
	}

	if(_hub_fd_lookup[file]==NULL) {
	        DEBUG("That file descriptor (%i) is invalid: perhaps you didn't call open first? Assigning hub fileops\n", file);
	        DEBUG_FILE("That file descriptor (%i) is invalid: perhaps you didn't call open first? Assigning hub fileops\n", file);
		_hub_fd_lookup[file] = _hub_fileops;
		//errno = EBADF;
		//return -1;
	}
	
	DEBUG_FILE("CALL: _hub_CLOSE. Fd = %d\n", file);
...
RETT_DUP _hub_DUP(INTF_DUP)
{
	...
	if(_hub_fd_lookup[file]==NULL) {
		DEBUG_FILE("That file descriptor (%i) is invalid: perhaps you didn't call open first?\n", file);
		errno = EBADF;
		return -1;
	}
	...
	DEBUG_FILE("_hub_DUP  is calling %s->DUP\n", _hub_fd_lookup[file]->name);
	...
	if(result >= 0) {
		...
		DEBUG_FILE("Hub(dup) managing new FD %i with same ops (\"%s\") as initial FD (%i)\n", result, _hub_fd_lookup[file]->name, file);
		_hub_fd_lookup[result] = _hub_fd_lookup[file];
	}

	//_hub_fd_lookup[CALL_DUP] = _hub_fileops;
...
RETT_DUP2 _hub_DUP2(INTF_DUP2)
{
	...
	if(_hub_fd_lookup[file]==NULL) {
		DEBUG_FILE("The first file descriptor (%i) is invalid: perhaps you didn't call open first?\n", file);
		errno = EBADF;
		return -1;
	}
	...
	if(_hub_fd_lookup[fd2]==NULL) {
	        MSG("The second file descriptor (%i) is invalid: perhaps you didn't call open first?\n", fd2);
		errno = EBADF;
		return -1;
	}
	*/
	...
	if( _hub_fd_lookup[file] != _hub_fd_lookup[fd2] ) {
		...
		DEBUG("_hub_DUP2: fd1 (%i) and fd2 (%i) have the same handler (%s)\n", file, fd2, _hub_fd_lookup[file]->name);
	}
	
	DEBUG_FILE("_hub_DUP2 is calling %s->DUP2(%i, %i) (p=%p)\n", _hub_fd_lookup[file]->name, file, fd2, _hub_fd_lookup[file]->DUP2);
	...
	if(result < 0)
	{
		DEBUG_FILE("DUP2 call had an error.\n");
		WARNING("fd2 (%i) may not be correctly marked as valid/invalid in submodules\n", fd2);
	}
	else
	{ 
	        DEBUG_FILE("DUP2 call completed successfully.\n");
		
		if(result != fd2)
		{
			DEBUG_FILE("_hub_DUP2: result!=fd2 (%i != %i), so let's update the table...\n", result, fd2);
			_hub_fd_lookup[fd2] = NULL;
		}

		DEBUG_FILE("Hub(dup2) managing new FD %i with same ops (\"%s\") as initial FD (%i)\n", result, _hub_fd_lookup[file]->name, file);
...
RETT_FCNTL _hub_FCNTL(INTF_FCNTL) {
	...
	CHECK_RESOLVE_FILEOPS(_hub_);
        ...
        DEBUG("CALL: _hub_FCNTL for %d\n", file);

	struct Fileops_p* op_to_use = NULL;
	int result = 0, num_params = 0;
	va_list arg;
	int arg3_int = 0;
	struct flock *arg3_flock = NULL;
	struct f_owner_ex *arg3_fowner_ex = NULL;
	uint64_t *arg3_uint64 = NULL;
	...
	case F_SETFL:
		...
		va_end(arg);		
		if (_hub_fd_lookup[file] == NULL) {
			DEBUG("%s: All fileops null for fd = %d. Setting to posix\n", __func__, file);
			op_to_use = _hub_fileops;
		} else
			op_to_use = _hub_fd_lookup[file];
		break;

	case F_SETLK:
		num_params = 3;
		va_start(arg, cmd);
		arg3_flock = va_arg(arg, struct flock *);
		va_end(arg);
		op_to_use = _hub_fileops;
		break;
	...
	case F_GETOWN_EX:
		num_params = 3;
		va_start(arg, cmd);
		arg3_fowner_ex = va_arg(arg, struct f_owner_ex *);
		va_end(arg);
		op_to_use = _hub_fileops;
		break;
	...
	case F_GET_RW_HINT:
		num_params = 3;
		va_start(arg, cmd);
		arg3_uint64 = va_arg(arg, uint64_t *);
		va_end(arg);
		op_to_use = _hub_fileops;
		break;
	...
	*/
	...
	if (num_params == 2) {
		result = op_to_use->FCNTL(file, cmd);
	} else {

		if (arg3_uint64 != NULL)       
			result = op_to_use->FCNTL(file, cmd, arg3_uint64);
		else if (arg3_fowner_ex != NULL)
			result = op_to_use->FCNTL(file, cmd, arg3_fowner_ex);
		else if(arg3_flock != NULL)
			result = op_to_use->FCNTL(file, cmd, arg3_flock);
		else {
			result = op_to_use->FCNTL(file, cmd, arg3_int);
			DEBUG("%s: arg 3 = %d\n", __func__, arg3_int);
	...
	if (result == -1) {
		DEBUG_FILE("%s: FCNTL failed.\n", __func__);
		assert(0);
	}

	DEBUG("%s: fcntl returned %d\n", __func__, result);
	return result;
}
