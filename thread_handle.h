...
//uint64_t anon_mem_allocated;
//uint64_t total_num_alloc_anon_mmaps;
//uint64_t num_files_closed;
//int num_files_closed;

uint64_t lim_anon_mem_closed;
uint64_t lim_num_files;
uint64_t lim_anon_mem;
...
static void bgCloseFiles(int main_thread) {
	...
	while(num_files_closed) {		
		if (anon_mem_closed_files <= (500*1024*1024) && !cleanup) {
			//printf("%s: setting async closing to 1\n", __func__);
			ASYNC_CLOSING = 1;
			break;
		}
		...
		printf("%s: acquiring lock for clf entry = %d\n", __func__, i);
		fflush(NULL);
#endif
...
#if PRINT_CONTENTION_MSGS		
		printf("%s: releasing lock for clf entry = %d\n", __func__, i);
		fflush(NULL);
#endif
	        LRU_NODE_UNLOCK_WR(clf);
	}
...
void checkAndActivateBgThread() {
	...
	//if(anon_mem_closed_files > lim_anon_mem_closed || cleanup) {
	if(anon_mem_closed_files > lim_anon_mem_closed || num_files_closed > lim_num_files || cleanup) {
		calledBgThread++;
		activateBgThread();	
