...
enum instrumentation_vars {
	...
	write_syscall_t,
	nvp_do_pwrite_t,
	nvp_do_pwrite_start_t,
	nvp_do_pwrite_overwrite_t,
	nvp_do_pwrite_append_t,
	nvp_do_pwrite_loop_t,
	seek_t,
	fsync_t,
	unlink_t,
	bg_thread_t,
...
const char *Instruprint[INSTRUMENT_NUM] =
{
	...
	"nvp_do_pwrite",
	"nvp_do_pwrite_start",
	"nvp_do_pwrite_overwrite",
	"nvp_do_pwrite_append",
	"nvp_do_pwrite_loop",
	"seek",
	"fsync",
	"unlink",
...
#define PRINT_TIME()							\
	{								\
		int i;							\
		RESULT_FILE("====================== NVP TIMING stats: ======================\n"); \
		for(i=0; i<INSTRUMENT_NUM; i++)	{			\
			if (Instrustats[i] != 0) {			\
				MSG("%s: timing = %lu nanoseconds\n",	\
				    Instruprint[i], Instrustats[i]);	\
				RESULT_FILE("%s: timing = %lu nanoseconds\n", \
					    Instruprint[i], Instrustats[i]); \
			}						\
		}							\
	}
