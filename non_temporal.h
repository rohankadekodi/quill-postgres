...
memmove_nodrain_movnt_granularity(void *pmemdest, const void *src, size_t len)
{
        ...
        if ((uintptr_t)dest1 - (uintptr_t)src >= len) {
                ...
                if (cnt > 0) {
                        ...
                        src = (char *)src + cnt;
                        len -= cnt;
			printf("%s: here copied upto flush align boundary\n", __func__);
		}
